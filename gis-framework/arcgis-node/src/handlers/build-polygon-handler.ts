 ...
 */
export const buildPolygonHandler: ArcGisServiceHandlers['buildPolygon'] = (call, callback) => {
  try {
    const polylines: Polyline[] = call.request.esriJsonPolylines.map((lineJson: string) => esriJsonToPolyline(lineJson));
    const polygon = buildPolygon(polylines, call.request.coordinateSystemWkid);
    ...
    console.error({ error: error }, 'Error building polygon');
    callback(error, null);
  }
};
