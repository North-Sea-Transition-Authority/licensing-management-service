...
import { densifyLoxodromesAndCalculateArea, LineWithNavigationType } from '../geometric-operators/calculate-area-operator';
...
export const calculateAreaHandler: ArcGisServiceHandlers['calculateArea'] = async (call, callback) => {
  try {
    ...
    logger.error({ error: error }, 'Error calculating area');
