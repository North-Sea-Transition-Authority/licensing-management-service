...
class FeatureRestControllerTest extends AbstractControllerTest {
  ...
  void getCommandJourneyActiveFeatures_assertJsonMap() throws Exception {
    ...
    when(commandJourneyService.getCommandJourneyOrThrow(commandJourney.getId())).thenReturn(commandJourney);
    when(commandJourneyService.getActiveFeatures(commandJourney)).thenReturn(List.of(feature));
    when(polygonService.getPolygonsAsEsriJson(feature, true))
        .thenReturn(List.of(esriJsonPolygon1, esriJsonPolygon2));
    ...
    mockMvc.perform(get("/api/gis-framework/command-journey-features/{commandJourneyId}", commandJourney.getId()))
        .andExpect(status().isOk())
        .andExpect(content().json(objectMapper.writeValueAsString(expectedFeatureSetMap), JsonCompareMode.STRICT));
  }
  ...
  void getCommandJourneyActiveOutlineNodes() throws Exception {
    ...
    when(commandJourneyService.getCommandJourneyOrThrow(commandJourney.getId())).thenReturn(commandJourney);
    when(commandJourneyService.getActiveFeatures(commandJourney)).thenReturn(List.of(feature1, feature2));
    when(lineService.getOutlineNodes(List.of(feature1, feature2))).thenReturn(featureOutlineNodes);

    mockMvc.perform(get("/api/gis-framework/command-journey-outline-nodes/{commandJourneyId}", commandJourney.getId()))
        .andExpect(status().isOk())
