...
function mockEndpoints() {
  worker.use(
    http.get("/api/features/1", () => HttpResponse.json(splitResultEd50)),
...
describe("merge page", () => {
  it("renders the map, textual description and a checkbox per feature", async () => {
    mockEndpoints();
    ...
    // Feature checkboxes and the textual description load asynchronously after mount — wait for both
    // before the screenshot so the baseline is deterministic.
    await expect.poll(() => document.querySelectorAll("input[type=checkbox]").length).toBe(2);
    await expect.poll(() => document.querySelector(".gis-textual-description")).not.toBeNull();

    await expect(screen.locator).toMatchScreenshot("merge-page");
  });
});
